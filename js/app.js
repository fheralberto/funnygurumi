...
function mostrarGaleria(max) {
  ...
  for (let i = 0; i < max; i++) {
    ...
    imagenItem.setAttribute("src", `img/galeria/${datosGaleria[i].nombre}.png`);
    imagenItem.setAttribute("alt", `${datosGaleria[i].alt}`);

    //Le crea atributo data-imagen-id = "i" 
    imagenItem.dataset.imagenId = datosGaleria[i].nombre;
...
function mostrarTodos() {
  ...
  datosGaleria.forEach(imagen => {
    ...
    autor.textContent = `Autor: ${imagen.autor}`
